...
│   ├── track-test.html/js  #   трек-движок/рекордер + folder/collapse + full-song clips + setClipEvents + inserts/device chain + audition when + record mode/quantize + chaseToTick + midiChannel routing + pitch bend/mod/sustain + clip audio playback + crossfade command (125/125)
...
│   ├── arranger-test.html/js #   arranger layout + clips + mini-notes + trim/split/dup/loop + multi-select + markers + mute/solo + rename + reorder + color + monitor + resize + folder/collapse + piano roll UI + quantize + transpose + duplicate + legato + fixed length + humanize + preview + step input + audio clips/waveform/editor + x-fade (217/217)
...
│   ├── audioInput-test.html/js # input devices, monitor gate, meter, take REC (14/14, нужны fake-mic флаги)
