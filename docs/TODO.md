...
- [x] Запись в выбранный клип по абсолютной позиции песни; сохранить исходные timestamps, длительности через несколько циклов, velocity и channel. Quantize должен быть обратимым. Готово 2026-09-12: штампы несут channel + абсолютные тики песни (`_songTicksNow`: транспорт с развёрткой лупов / часы движка), song-коммит пишет в выбранный клип в clip-relative координатах с полными длительностями, открытые ноты переживают wrap-коммиты (холд через циклы = одно событие), pattern-путь и stop-with-hold без изменений, квантованные тейки хранят `rawStart` + `engine.unquantizeClip`. track-test 147/147 (6 новых), полный юнит 761/761, app check чистый.
- [x] Запись одной undo-транзакцией, включая REPLACE, cancel take и остановку с удержанными нотами. Готово 2026-09-12: `recordTakeCommand` (компаунд по трекам, пересоздание тейк-клипов на redo), снапшоты при первой записи в клип, один `history.execute` на финальный коммит (промежуточные пишут напрямую, apply идемпотентен), `engine.cancelTake` (откат REPLACE-очистки, без следа в history), кнопка ✕ CANCEL в рекордере, history проброшена в движок. track-test 152/152 (5 новых), полный юнит 766/766, app check чистый.
- [x] Стабилизировать асинхронный browser test аудиозаписи: ожидание завершённого take вместо фиксированных задержек, диагностировать редкие сбои fake device под нагрузкой. Готово 2026-09-12: `takeSerial` в inputUI (все выходы finishTake + геттер), хелперы `waitFor`/`waitForTake`/`refreshDevicesReady`/`selectDeviceReady` (ретраи enumerate/select, диагностика статуса в сообщениях), фиксированные слипы 300/400/900/1200мс заменены ожиданиями сигнала и завершения тейка, cleanup через finally (протечки контекстов = нагрузка). audioInput 25/25 ×3 прогона, полный юнит 766/766, app check чистый.
- [x] MIDI: panic/all-notes-off, release при отключении устройства, идентичность device/channel/note, запись/воспроизведение CC/bend/pressure. Готово 2026-09-12: deviceId сквозит в колбэках, CC123 → `engine.panic()` (+кнопка PANIC, глушит голоса, сбрасывает экспрессию, закрывает тейк-ноты), `onDeviceLost` отпускает ровно ноты отключённого устройства, held-ключи включают device, 0xD0 → `onPressure`/`routePressure`/`voice.pressure` (фолбэк на CC1), снапшоты bend/mod/pressure в тейках + реплей в шедулерах и chase. tests/midi-test 6/6, track-test 157/157 (5 новых), полный юнит 777/777, app check чистый. Примечание: один раз флейкнул transport-тест pause/resume в полном прогоне под нагрузкой (в изоляции и повторах 56/56).
- [x] Chase CC/program/expression/pedal state. Имеющийся note chase не закрывает этот сценарий. Готово 2026-09-12: новые голоса наследуют живой mod/pressure на фильтр (`_filterTarget`), chase при нажатой педали держит ретриггер открытым (`holdForSustain`, отпуск педали гасит), 0xC0 → `onProgram`/`routeProgram` (выбор wave), снапшот `pgm` в тейках + реплей в шедулерах и chase, тейки переживают note-off при педали (закрытие на CC64-up). midi-test 7/7, track-test 161/161 (4 новых), полный юнит 782/782, app check чистый.
- [x] Голоса: непрерывный release из уровня attack/decay, voice stealing при lookahead, повтор одной высоты под sustain, modulation на новых голосах; звуковые regression tests. Готово 2026-09-12: analytic release level (`envelopeLevelAt` + `attackAt/attackVel`, `_releaseVoice` для noteOff/sustain/restrike) + `cancelAndHoldAtTime` там где есть (иначе атака стиралась до релиза — поймано offline-тестом); stealing сравнивает с временем ноты, а не now; рестрайк режет same-pitch (mod на новых голосах — из прошлого пункта). tests/voice-test 8/8 (mock + OfflineAudioContext: no-click, restrike peaks, stealing, уровни), полный юнит 790/790 (23 сьюта), app check чистый. Примечание: app-test раз флейкнул на холодном старте полного прогона (в изоляции и повторе 11/11).
...
- [x] Revision-based autosave, dirty/saved/error status, журнал, резервные снимки и recovery. Убрать MutationObserver и периодическую запись неизменённого состояния; flush перед уходом. Готово 2026-09-12: ревизии FNV-1a (skip без изменений, restore сидирует), кольцо 5 бэкапов + `recoverSnapshot` (boot recovery со статусом), журнал 50 + `getSaveState`/`subscribe` → строка статуса, `flush` на beforeunload/pagehide, `setOnMutate` рэка+роутера вместо observer/интервала. projectStore-test 19/19 (8 новых), полный юнит 806/806, app check чистый. Плюс: pause-тесты ужесточены `_clearTimer` после третьего флейка transport-теста в полных прогонах (10/10 после).
- [ ] Полная валидация: конечные числа, диапазоны, уникальные ID, ссылки на assets/devices/tracks, версии схемы. Сначала подготовить проект, затем атомарно заменить состояние. Реализация подготовлена 2026-09-13: строгая проверка до нормализации, независимая копия документа, подготовка рэка/голосов с освобождением частичных аудиографов, защита restore/recovery. Model suites 375/375; rack preparation 5/5 с mock DOM/AudioContext. Пункт остаётся открытым до полного browser/UI-прогона: Chromium недоступен в окружении разработки. Подробности и команды — [PROJECT_VALIDATION.md](PROJECT_VALIDATION.md).
