...
- [ ] Завершить переход на clip.events: удалить legacy grid/rt backing и syncLoopClip, оставить сетку только проекцией. Перенести оставшиеся прямые updateTrack/grid и recording paths на clip commands.
- [ ] В PATTERN выбирать воспроизводимый паттерн по стабильному clip ID, а не позиции 0. Устранить привязку первой области записи к нулю; выбор step-клипа уже независим от playback/recording.
- [ ] Недеструктивный trim/split/loop MIDI с source offset; явная политика для нот через разрез. События за правой границей клипа не звучат.
- [ ] Отмена и перепланирование событий при edit/delete/undo во время playback. Изменённые данные должны соответствовать звучанию.
- [ ] Единые musical-time преобразования. Текущий PPQ — 480; transport имеет tempo map, но части audio/MIDI/recording используют постоянный BPM и 4/4. Не менять PPQ без миграции.
- [ ] Seek/loop/pause/resume: исключить дубли chase, повтор законченных нот при late tick, ошибки RT-позиции и прерывание удержанных нот на границе.
- [ ] Подключить PatternSequencer рэка к общему транспорту; менять частоту осциллятора в момент запланированной ноты.
