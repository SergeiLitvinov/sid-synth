...
// 16-step grid (cells { note, dur } or legacy strings) -> clip events.
// Each event is { note, start, dur } in ticks, one per filled cell. `dur` is
// the cell's step count times one sixteenth; step index * one sixteenth.
export function gridToClipEvents(grid, { ppq = DEFAULT_PPQ } = {}) {
  ...
  (grid || []).forEach((cell, step) => {
    ...
    out.push({ note: c.note, start: step * st, dur: dur * st });
  });
  return out;
}

// Realtime notes (seconds) -> clip events in ticks.
...
// holding each event's quantized step is set to { note, dur }.
export function clipEventsToGrid(events, { ppq = DEFAULT_PPQ, steps = 16 } = {}) {
  ...
  (events || []).forEach(ev => {
    ...
    grid[step] = { note: ev.note, dur: durSteps };
  });
  return grid;
}
