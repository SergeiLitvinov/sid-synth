...
// Held notes are keyed by device + channel + note so two devices (or the
// keyboard) can hold the same pitch independently and a disconnect release
// frees exactly the notes that device was holding.
export function createLiveInput(engine, { stampOn, stampOff, emitNote }) {
  const held = new Map();
  const key = (device, note, channel) => `${device ?? 'local'}:${channel ?? 'keyboard'}:${note}`;
  ...
  function noteOn(note, { channel = null, velocity = 100, device = null } = {}) {
    if (engine.ctx.state === 'suspended') engine.ctx.resume();
    const name = String(note).toUpperCase();
    const recipients = targets(channel);
    const id = key(device, name, channel);
    ...
    recipients.forEach(t => {
      ...
      if (engine._recording && engine._armed.has(t.id)) stampOn(t, name, velocity, channel, device);
    });
    emitNote(name);
  }

  function noteOff(note, { channel = null, device = null } = {}) {
    const name = String(note).toUpperCase();
    const id = key(device, name, channel);
    const queue = held.get(id);
    ...
    ids.forEach(trackId => {
      ...
      if (engine._recording) stampOff(t, name, channel, device);
    });
  }
