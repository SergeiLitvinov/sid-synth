...
check('x-fade button crossfades two selected overlapping audio clips (undoable)', () => {
  const container = document.createElement('div');
  const ctx = createMockAudioContext();
  const engine = createTrackEngine(ctx, ctx.destination, { bpm: 120 });
  engine.addTrack({ id: 'trk_a', name: 'A' });
  engine.activeTrackId = 'trk_a';
  const a = engine.addClip('trk_a', { start: 0, length: 1920 });
  const b = engine.addClip('trk_a', { start: 960, length: 1920 });
  engine.setClipAudio('trk_a', a.id, { hash: 'h1' });
  engine.setClipAudio('trk_a', b.id, { hash: 'h2' });
  const history = createHistory();
  createArranger({ container, engine, transport: fauxTransport(), history });
  const content = container.querySelector('.arranger-content');
  content.getBoundingClientRect = () => ({ left: 0, top: 0, right: 800, bottom: 100, width: 800, height: 100 });
  const blocks = container.querySelectorAll('.arranger-clip');
  blocks[0].dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, pointerId: 1, clientX: 0 }));
  blocks[1].dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, pointerId: 1, clientX: 96, ctrlKey: true }));
  if (container.querySelectorAll('.arranger-clip.selected').length !== 2) return false;
  const btn = [...container.querySelectorAll('.arranger-btn')].find(x => x.textContent === 'x-fade');
  if (!btn) return false;
  btn.click();
  const ra = engine.byId.trk_a.clips.find(c => c.id === a.id).audio;
  const rb = engine.byId.trk_a.clips.find(c => c.id === b.id).audio;
  if (!(ra.fadeOut === 1 && rb.fadeIn === 1)) return false;
  history.undo();
  const ua = engine.byId.trk_a.clips.find(c => c.id === a.id).audio;
  const ub = engine.byId.trk_a.clips.find(c => c.id === b.id).audio;
  return ua.fadeOut === 0 && ub.fadeIn === 0;
});
check('x-fade button is a no-op without two overlapping audio clips', () => {
  const container = document.createElement('div');
  const ctx = createMockAudioContext();
  const engine = createTrackEngine(ctx, ctx.destination, { bpm: 120 });
  engine.addTrack({ id: 'trk_a', name: 'A' });
  engine.activeTrackId = 'trk_a';
  const a = engine.addClip('trk_a', { start: 0, length: 960 });
  const b = engine.addClip('trk_a', { start: 960, length: 960 });
  engine.setClipAudio('trk_a', a.id, { hash: 'h1' });
  engine.setClipAudio('trk_a', b.id, { hash: 'h2' });
  const history = createHistory();
  createArranger({ container, engine, transport: fauxTransport(), history });
  ...
  btn.click();
  const ra = engine.byId.trk_a.clips.find(c => c.id === a.id).audio;
  return ra.fadeOut === 0 && history.state().canUndo === false;
});
